...
public class OpenbisV3CreationController implements IOpenbisCreationController {
  ...
  public void registerExperimentsWithSamples(String space, String project,
      List<RegisterableExperiment> exps, final Runnable ready) {
    ...
    Thread t = new Thread(new Runnable() {
      ...
      public void run() {
        ...
        List<ISampleBean> batch = new ArrayList<>();
        for (RegisterableExperiment e : exps) {
          batch.addAll(e.getSamples());
        }
        if (!registerSampleBatch(batch)) {
          errors = "Samples could not be registered.";
          Log.error(errors);
          UI.getCurrent().setPollInterval(-1);
          UI.getCurrent().access(ready);
          return;
